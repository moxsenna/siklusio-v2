...
export function CreditDetailModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const handleTopUp = async (pkg: TopUpPackage) => {
    ...
    try {
      const data = await apiPostJson<{ paymentUrl?: string; error?: string }>(
        "/api/checkout/topup",
        {
          packageId: pkg.id,
        },
      ...
      <View className="flex-1 justify-end">
        ...
        <View className="relative bg-white rounded-t-[32px] w-full max-w-md mx-auto p-[24px] pb-[40px] shadow-xl z-50 min-h-[70%]">
          ...
          <View className="bg-purple-100 rounded-[20px] p-5 items-center mb-6 border border-purple-200">
            <Text className="text-purple-600 text-sm font-bold uppercase tracking-widest mb-1">
              Saldo Saat Ini
            </Text>
            {loading && balance === null ? (
              <ActivityIndicator color="#9333ea" />
            ) : (
              <Text className="text-4xl font-extrabold text-purple-800">
                {balance ?? "-"} <Text className="text-xl">✨</Text>
              </Text>
            )}
          </View>
          ...
          <View className="flex-row bg-gray-100 rounded-full p-1 mb-6">
            ...
            >
              <Text
                className={`font-bold ${activeTab === "topup" ? "text-purple-700" : "text-gray-500"}`}
              >
                Top-Up
              </Text>
            </TouchableOpacity>
            ...
            >
              <Text
                className={`font-bold ${activeTab === "history" ? "text-purple-700" : "text-gray-500"}`}
              >
                Riwayat
              </Text>
            </TouchableOpacity>
            ...
            {activeTab === "topup" ? (
              <View className="gap-3 pb-8">
                {PACKAGES.map((pkg) => (
                  <View
                    key={pkg.id}
                    className="bg-white border border-purple-100 rounded-2xl p-4 flex-row items-center justify-between shadow-sm"
                  >
                    <View>
                    ...
                    >
                      <View className="flex-1 pr-3">
                        <Text className="text-sm font-bold text-gray-700">
                          {item.feature === "topup" ? "Top-Up" : "Pemakaian"}: {item.reason}
                        </Text>
                      ...
                      <Text
                        className={`font-bold text-base ${item.amount > 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        {item.amount > 0 ? "+" : ""}
                        {item.amount}
                      </Text>
