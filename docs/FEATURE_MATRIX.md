...
2. Legacy/free-included AI features that require auth and rate limit, but do not debit AI credits or persist generated results yet.
...
The free-included legacy pattern is:

1. Require user auth with `requireUser(c)`.
2. Use the AI route rate limit group.
3. Call OpenRouter with free models only.
4. Do not pass `OPENROUTER_PAID_MODEL` as fallback.
5. Return validated JSON without charging or persistence.
6. Revisit pricing only together with persistence, credit labels, and mobile 402 handling.
...
3. If the feature is free, it must use free OpenRouter models only and should have UI copy saying it is included/free.
...
3. Should TWW reassurance remain free-included because it is emotionally sensitive, or become paid after adding minimal consent/audit metadata?
...
6. Should free-included features get a stricter per-user quota beyond the current shared AI route rate limit?
