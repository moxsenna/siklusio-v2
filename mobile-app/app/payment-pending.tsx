...
export default function PaymentPendingScreen() {
  ...
  const [message, setMessage] = useState('Akunmu sudah dibuat, tapi akses premium masih menunggu konfirmasi pembayaran Mayar.');
  ...
  const refreshPaymentStatus = async () => {
    ...
    try {
      ...
      setMessage('Pembayaran belum terkonfirmasi. Jika baru saja membayar, tunggu sebentar lalu cek ulang.');
    } catch (error: any) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fdf2f8' }}>
      <View style={{ flex: 1, justifyContent: 'center', padding: 24 }}>
        <View style={{ backgroundColor: '#ffffff', borderRadius: 28, padding: 24, borderWidth: 1, borderColor: '#fbcfe8' }}>
          ...
          <TouchableOpacity
            ...
            disabled={isChecking}
            style={{ backgroundColor: '#ec4899', borderRadius: 18, paddingVertical: 14, alignItems: 'center', marginBottom: 10 }}
          >
            {isChecking ? (
              <ActivityIndicator color="#ffffff" />
            ) : (
              <Text style={{ color: '#ffffff', fontWeight: '800' }}>Saya Sudah Bayar, Cek Ulang</Text>
            )}
          </TouchableOpacity>

          <TouchableOpacity
            onPress={signOut}
            style={{ borderColor: '#f9a8d4', borderWidth: 1, borderRadius: 18, paddingVertical: 14, alignItems: 'center' }}
          >
