...
**Web smoke script** (`node scripts/sprint3-mobile-qa-web.mjs`):

- **exit 0** — functional routes render; login OK (warnings/known issues OK)
- **exit 1** — functional blocker only (login fail, route crash, uncaught page error, blocker test FAIL)
- Console `collapsable` / asset 404 → recorded in `qa-results.json` as `knownIssues`, non-blocking
